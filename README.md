...
Hyungtaek Jung et al.: **_EasyFM_**: An **Easy** software suite for **F**ile **M**anipulation of Next Generation Sequencing data, [biorxiv]().
...
### 3.2 INSTALLATION
**Windows**: Please download the program from [this link](https://github.com/OZTaekOppa/EasyFM/blob/main/downloads/easyfm.7z). Extract 'easyfm.7z' with [7z](https://www.7-zip.org/download.html) and double-click easyfm.exe. 

**!!! Please note**, for antivirus issues, please follow these steps: 1) click "undo action token" from "other actions", 2) click "start undo", and 3) re-install the program. Alternatively, if you see this dialogue box "Suspicious File Warning", "It contains a suspicious file. Do you want to proceed with decompression?", please just click "Yes". For any other issues, we highly encourage users to use the [Issues](https://github.com/OZTaekOppa/EasyFM/issues).
...
![Figure 1: Integration of secondary window with main work module of EasyFM](./images/figure1.png)
...
* **Basic Local Alignment Search Tool (BLAST)**       # All BLAST output file formats including an xml file parsing feature
...
The BLAST module is designed to easily format and run searches on a local computer and database. Since any fasta files can be used for BLAST functions without relying on the NCBI web server, running local BLAST via EasyFM shows great advantages to using customized databases including your own unpublished genome and transcriptome sequences that are not available on GenBank. Figure 2 shows an easy step for each stage for the BLAST module.

![Figure 2: User-friendly standalone work modules in EasyFM: BLAST module](./images/figure2.png)
...
![Figure 4: User-friendly standalone work modules in EasyFM: ORF module](./images/figure4.png)
...
### 3.5 FAQ
We encourage users to use the [Issues](https://github.com/OZTaekOppa/EasyFM/issues). For further enquiries (including suggestions).
